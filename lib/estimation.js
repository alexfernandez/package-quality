...
exports.addQuality = function(estimation)
{
	...
	for (var factor in estimation)
	{
		if (estimation[factor] instanceof Array && estimation[factor].length === 2)
		{
			...
			if (quality === 0)
			{
				estimation.nextUpdate = moment(estimation.nextUpdate).add(1, 'month').format();
				return estimation;
			}
			numerator += weight;
 ...
 **************** UNIT TESTS ********************
 ************************************************/
function testEstimate(callback)
{
...
function testPending(callback)
{
	...
	// stub factors.fakeFactor
	factors.fakeFactor = {
		pending: function (pendingItem, internalCallback) {
			testing.assertEquals(pendingItem.check, 1, 'wrong item sent to factor pending', callback);
			internalCallback(null, {fake: 2});
		}
	};
	exports.pending(pending, function(error, result) {
		testing.check(error, callback);
		testing.check(result.check, callback);
		testing.check(result.factor, callback);
		testing.assertEquals(result.fake, 2, 'fake was not added in pending', callback);
		testing.success(callback);
	});
}
