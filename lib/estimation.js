...
exports.estimate = function(entry, callback)
{
	...
	var result = {
		name: entry.name,
		source: 'npm',
		githubApiRequest: true
	};
	var waterfall = [
		// estimate repo 
		function(callback)
		{
			return estimateRepo(entry.repository, function(error, repoQuality) {
				if (error)
				{
					result.githubApiRequest = false;
				...
				result.repoQuality = repoQuality;
				return callback(error, repoQuality);
			});
		},
