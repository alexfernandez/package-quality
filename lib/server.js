...
function serveBadge (request, response) {
	...
	findPackage(packageName, function(error, result) {
		...
		badges.compileBadge(packageName, quality.toFixed(2), size, function (error, png) {
			...
			response.send(png);
			const url = 'https://librecounter.org/count?url=https://packagequality.com/badge/' + packageParam
			request.get(url, function(error) {
				if (error) {
					console.error('Could not count visit to badge', error)
				}
			})
		});
	});
...
function serveShield(request, response) {
	...
	findPackage(packageName, function(error, result) {
		...
		badges.retrieveShield(packageName, score, queryString, function (error, svg) {
			...
			response.send(svg);
			const url = 'https://librecounter.org/count?url=https://packagequality.com/shield/' + packageParam
			request.get(url, function(error) {
				if (error) {
					console.error('Could not count visit to shield', error)
				}
			})
		});
	});
...
function serve (request, response) {
	...
	readPackage(npmPackage, function(error, estimation) {
		...
		const url = 'https://librecounter.org/count?url=https://packagequality.com/' + npmPackage
		request.get(url, function(error) {
			if (error) {
				console.error('Could not count visit to package', error)
			}
		})
	})
}
