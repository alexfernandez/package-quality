 ...
 */
var getIssuesForPageLastYear = function (repoOwner, repoName, page, callback)
{
    var result = {issues:[]};
...
function testEstimateValidRepoPending(callback)
{
    ...
    exports.estimate(validRepo, function (error, result) {
        testing.check(error, callback);
        testing.assertEquals(result.pending.owner, 'alexfernandez', 'wrong repo owner returned in pending', callback);
        testing.assertEquals(result.pending.name, 'loadtest', 'wrong repo name returned in pending', callback);
        testing.assertEquals(result.pending.pages[0], 2, 'wrong initial page returned in pending', callback);
        testing.assertEquals(result.pending.pages[1], 3, 'wrong final page returned in pending', callback);
        testing.assertEquals(result.pending.total, 1, 'wrong total issues returned in pending', callback);
        testing.assertEquals(result.pending.open, 1, 'wrong open issues returned in pending', callback);
        testing.assertEquals(result.pending.closed, 0, 'wrong closed issues returned in pending', callback);
        testing.assertEquals(result.pending.longOpen, 1, 'wrong long open issues returned in pending', callback);
        testing.success(callback);
    });
}
...
function testPending(callback)
{
    ...
    exports.pending(pending, function (error, result) {
        testing.check(error, callback);
